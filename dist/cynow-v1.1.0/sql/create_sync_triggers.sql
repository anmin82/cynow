-- cy_cylinder_current 자동 동기화 Trigger 및 함수
...
BEGIN
    ...
    v_status := CASE 
        ...
        WHEN v_condition_code IN ('950', '952') THEN '정비'  -- 정비 후 공용기로 사용 가능
        WHEN v_condition_code = '990' THEN '폐기'  -- 실제 폐기된 용기
        ELSE '기타'
    END;
    
    -- EndUser 결정 (예외 우선, 기본값 차순)
    -- cy_enduser_exception의 cylinder_no도 TRIM된 값으로 저장되어야 하므로 RTRIM으로 비교
    -- 정책이 매칭되지 않으면 NULL로 설정 (기본값 SDC 사용 안 함)
    SELECT COALESCE(
        ...
        (SELECT default_enduser FROM cy_enduser_default 
         ...
         LIMIT 1),
        NULL  -- 정책이 없으면 NULL (기본값 SDC 사용 안 함)
    ) INTO v_enduser;
    ...
    -- cylinder_type_key 생성 (Dashboard 값 기준 - 번역된 값 사용)
    ...
    -- cylinder_type_key_raw 생성 (Raw 값 기준)
    ...
    -- 가용 여부 (보관, 충전만 가용, 정비는 정비 후 사용 가능하므로 비가용)
    v_is_available := v_status IN ('보관', '충전');
    
    -- Upsert (용기번호는 TRIM된 값 사용)
...
-- 삭제 처리 함수
CREATE OR REPLACE FUNCTION delete_cylinder_current(p_cylinder_no VARCHAR)
RETURNS VOID AS $$
DECLARE
    v_cylinder_no_trimmed VARCHAR(20);
BEGIN
...
-- Trigger 함수 (ma_cylinders 변경 시)
CREATE OR REPLACE FUNCTION trigger_sync_cylinder_current_cylinders()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'DELETE' THEN
        PERFORM delete_cylinder_current(OLD."CYLINDER_NO");
        RETURN OLD;
    ELSE
        PERFORM sync_cylinder_current_single(NEW."CYLINDER_NO");
        RETURN NEW;
    END IF;
END;
$$ LANGUAGE plpgsql;

-- Trigger 함수 (tr_latest_cylinder_statuses 변경 시)
CREATE OR REPLACE FUNCTION trigger_sync_cylinder_current_status()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'DELETE' THEN
        -- tr_latest_cylinder_statuses 삭제 시에는 스냅샷에서 삭제하지 않음
        -- (ma_cylinders가 있으면 여전히 존재하는 용기이므로)
        RETURN OLD;
    ELSE
        PERFORM sync_cylinder_current_single(NEW."CYLINDER_NO");
        RETURN NEW;
...
AFTER INSERT OR UPDATE OR DELETE ON "fcms_cdc"."ma_cylinders"
FOR EACH ROW EXECUTE FUNCTION trigger_sync_cylinder_current_cylinders();
...
AFTER INSERT OR UPDATE OR DELETE ON "fcms_cdc"."tr_latest_cylinder_statuses"
FOR EACH ROW EXECUTE FUNCTION trigger_sync_cylinder_current_status();
